...
Creates a two-page planning app with:
  ...
  * Create-plan, add-forecast-entry, and review-decision modal overlays
  * Insert / update / delete row actions — every write goes through a button
    + control(s), never a raw cell edit (see approval-workflow-pattern.md's
    inputMode section for why)
...
def build_spec(connection_id, folder_id):
        ...
        "actions": [{
            ...
            "effects": [
                {"effect": "set-control-value", "control": "selected_plan_id",
                 "value": {"type": "column", "columnId": "rv-plan-id"}},
    ...
    controls = [
        ...
        # Explicit submit target: typed/pasted by the planner rather than
        # inferred from whatever the shared selected_plan_id happens to hold
        # (that control is only reliably set by the create flow, or by having
        # visited the Review page and clicked a row first — a silent trap on
        # the Forecast page for any plan you didn't just create).
        {"id": "ctrl-submit-id", "kind": "control",
         "controlId": "submit_plan_id", "name": "Plan ID to submit",
         "controlType": "text", "case": "insensitive", "mode": "equals",
         "value": "", "includeNulls": "when-no-value-is-selected",
         "showOperators": False},
        # New-forecast-entry controls. A button+modal insert-rows path, not
        # raw cell editing — see the inputMode note below.
        {"id": "ctrl-fc-plan-id", "kind": "control",
         "controlId": "fc_new_plan_id", "name": "Plan ID",
         "controlType": "text", "case": "insensitive", "mode": "equals",
         "value": "", "includeNulls": "when-no-value-is-selected",
         "showOperators": False},
        {"id": "ctrl-fc-period", "kind": "control",
         "controlId": "fc_new_period", "name": "Period",
         "controlType": "date", "mode": "=",
         "value": "2026-01-01T00:00:00Z",
         "includeNulls": "when-no-value-is-selected"},
        {"id": "ctrl-fc-dimension", "kind": "control",
         "controlId": "fc_new_dimension", "name": "Dimension",
         "controlType": "text", "case": "insensitive", "mode": "equals",
         "value": "", "includeNulls": "when-no-value-is-selected",
         "showOperators": False},
        {"id": "ctrl-fc-baseline", "kind": "control",
         "controlId": "fc_new_baseline", "name": "Baseline",
         "controlType": "number", "mode": "=", "value": 0,
         "includeNulls": "when-no-value-is-selected"},
        {"id": "ctrl-fc-proposed", "kind": "control",
         "controlId": "fc_new_proposed", "name": "Proposed",
         "controlType": "number", "mode": "=", "value": 0,
         "includeNulls": "when-no-value-is-selected"},
        {"id": "ctrl-fc-comments", "kind": "control",
         "controlId": "fc_new_comments", "name": "Planner Comments",
         "controlType": "text-area", "value": ""},
    ]
    ...
    create_submit = button("btn-create-submit", "Create draft", [
        {"effect": "insert-rows", "tableElementId": "it-plan-registry",
         "values": {
    ...
    submit_review = button("btn-submit-review", "Submit plan for review", [
        {"effect": "update-rows", "tableElementId": "it-plan-registry",
         # Formula targeting avoids depending on the input table's system Row
         # ID, and submit_plan_id (typed on this page) avoids depending on
         # selected_plan_id, which is only reliably populated by the create
         # flow or by a prior visit to the Review page's row-select.
         "whichRows": {"type": "formula",
                       "formula": "[Plan ID] = [submit_plan_id]"},
         "values": {
             "pr-status": {"type": "constant",
    ...
    add_forecast_button = button("btn-add-forecast", "Add forecast entry", [
        {"effect": "open-overlay", "overlayId": "modal-add-forecast"},
    ], "#0d6efd")

    add_forecast_submit = button("btn-add-forecast-submit", "Add entry", [
        # Button + insert-rows, not a raw cell edit — see the inputMode note
        # in approval-workflow-pattern.md. Confirmed live 2026-08-12 that a
        # fresh-from-API input table with inputMode:"view" still rejects
        # direct cell typing in default (non-edit) view on this org; routing
        # every write through an action avoids depending on that being fixed.
        {"effect": "insert-rows", "tableElementId": "it-forecast",
         "values": {
             "fc-plan-id": {"type": "control", "control": "fc_new_plan_id"},
             "fc-period": {"type": "control", "control": "fc_new_period"},
             "fc-dimension": {"type": "control", "control": "fc_new_dimension"},
             "fc-baseline": {"type": "control", "control": "fc_new_baseline"},
             "fc-proposed": {"type": "control", "control": "fc_new_proposed"},
             "fc-comments": {"type": "control", "control": "fc_new_comments"},
         }},
        {"effect": "refresh-element",
         "target": {"type": "element", "element": "it-forecast"}},
        {"effect": "close-overlay"},
    ], "#0d6efd")
    ...
    delete_plan = button("btn-delete-plan", "Delete plan", [
        {"effect": "delete-rows", "tableElementId": "it-plan-registry",
         "whichRows": {"type": "formula",
                       "formula": "[Plan ID] = [selected_plan_id]"}},
        {"effect": "refresh-element",
         "target": {"type": "element", "element": "tbl-review"}},
    ...
    cancel_review = button("btn-review-cancel", "Cancel",
                           [{"effect": "close-overlay"}],
                           "#ffffff", "#1e3558", "outline")
    cancel_add_forecast = button("btn-add-forecast-cancel", "Cancel",
                                 [{"effect": "close-overlay"}],
                                 "#ffffff", "#1e3558", "outline")
    ...
    elements = [
        ...
        {"id": "add-forecast-help", "kind": "text",
         "body": "**New forecast entry**\nUse the same Plan ID as the scenario this entry belongs to."},
        *controls,
        create_button, create_submit, submit_review, review_submit,
        delete_plan, cancel_create, cancel_review,
        add_forecast_button, add_forecast_submit, cancel_add_forecast,
    ]
        ...
        {"id": "modal-review", "type": "modal", "name": "Review Plan",
         "modal": {"width": "large",
                   "header": {"title": "Review Forecast Plan",
                              "showCloseIcon": "shown"}}},
        {"id": "modal-add-forecast", "type": "modal", "name": "Add Forecast Entry",
         "modal": {"width": "large",
                   "header": {"title": "Add Forecast Entry",
                              "showCloseIcon": "shown"}}},
    ]

    layout = """<?xml version="1.0" encoding="utf-8"?>
<Page type="grid" gridTemplateColumns="repeat(24, 1fr)" gridTemplateRows="auto" id="page-forecast">
  <Element elementId="title-forecast" gridColumn="1 / 13" gridRow="1 / 4"/>
  <Element elementId="btn-create-plan" gridColumn="13 / 17" gridRow="1 / 4"/>
  <Element elementId="btn-add-forecast" gridColumn="17 / 21" gridRow="1 / 4"/>
  <Element elementId="ctrl-submit-id" gridColumn="21 / 25" gridRow="1 / 3"/>
  <Element elementId="btn-submit-review" gridColumn="21 / 25" gridRow="3 / 5"/>
  <Element elementId="it-forecast" gridColumn="1 / 25" gridRow="5 / 25"/>
</Page>
...
<Page type="grid" gridTemplateColumns="repeat(24, 1fr)" gridTemplateRows="auto" id="modal-add-forecast">
  <Element elementId="add-forecast-help" gridColumn="1 / 25" gridRow="1 / 4"/>
  <Element elementId="ctrl-fc-plan-id" gridColumn="1 / 13" gridRow="4 / 7"/>
  <Element elementId="ctrl-fc-period" gridColumn="13 / 25" gridRow="4 / 7"/>
  <Element elementId="ctrl-fc-dimension" gridColumn="1 / 25" gridRow="7 / 10"/>
  <Element elementId="ctrl-fc-baseline" gridColumn="1 / 13" gridRow="10 / 13"/>
  <Element elementId="ctrl-fc-proposed" gridColumn="13 / 25" gridRow="10 / 13"/>
  <Element elementId="ctrl-fc-comments" gridColumn="1 / 25" gridRow="13 / 17"/>
  <Element elementId="btn-add-forecast-cancel" gridColumn="13 / 19" gridRow="17 / 20"/>
  <Element elementId="btn-add-forecast-submit" gridColumn="19 / 25" gridRow="17 / 20"/>
</Page>
"""
